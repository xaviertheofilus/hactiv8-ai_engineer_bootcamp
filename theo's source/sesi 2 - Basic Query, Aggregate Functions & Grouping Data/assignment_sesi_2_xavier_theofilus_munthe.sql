...
-- TASK 1.3
SELECT
    COUNT(*) AS jumlah_produk_discontinued
FROM products
...
SELECT
    c.category_name,
    ROUND(SUM(p.unit_price * p.units_in_stock)::NUMERIC, 2) AS total_nilai_stok
FROM products p
JOIN categories c ON p.category_id = c.category_id
GROUP BY c.category_name
...
JOIN categories c ON p.category_id = c.category_id
GROUP BY c.category_name
HAVING SUM(p.unit_price * p.units_in_stock) > 10000
ORDER BY total_nilai_stok DESC;


-- TASK 3.1
SELECT
    EXTRACT(YEAR  FROM order_date)::INT AS tahun,
    EXTRACT(MONTH FROM order_date)::INT AS bulan,
...
SELECT
    ship_country,
    ROUND(AVG(freight)::NUMERIC, 2) AS rata_rata_freight
FROM orders
GROUP BY ship_country
ORDER BY rata_rata_freight DESC
LIMIT 5;
...
-- Employee dengan order terbanyak adalah Margaret Peacock
-- dengan jumlah order 156

-- TASK 5

-- Kategori Beverages memiliki total nilai stok tertinggi, diikuti oleh
-- Meat/Poultry. Hal ini mengindikasikan bahwa perusahaan menyimpan modal
-- terbesar di produk minuman. Implikasinya, tim procurement dan gudang
-- perlu memantau perputaran stok Beverages secara ketat agar tidak terjadi
-- penumpukan modal yang berlebihan, sekaligus memastikan ketersediaan
-- produk yang paling diminati pelanggan.

-- Berdasarkan data order bulanan, terlihat tren peningkatan jumlah order
-- dari tahun 1996 ke 1998. Jumlah order per bulan cenderung naik secara
-- konsisten, dengan puncak tertinggi terjadi pada awal tahun 1998
-- (Januari 1998 ~55 order). Terdapat indikasi seasonal peak di
-- kuartal pertama setiap tahun, kemungkinan dipengaruhi oleh periode
-- awal tahun fiskal pelanggan. Sebaliknya, data bulan Mei 1998 sangat
-- rendah (sekitar 14 order) karena data hanya mencakup sebagian bulan
-- tersebut (data terpotong), bukan mencerminkan penurunan sesungguhnya.
 

-- Manajemen sebaiknya meningkatkan kapasitas distribusi menjelang kuartal
-- pertama tiap tahun, dan melakukan evaluasi ulang komposisi stok kategori
-- Beverages dan Meat/Poultry untuk menjaga efisiensi modal kerja.